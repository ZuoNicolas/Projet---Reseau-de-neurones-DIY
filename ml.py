...
class ReLu(Module):
    """ Module représentant une couche de transformation ReLu.
    """
    def __init__(self):
        """ Constructeur du module SoftMax.
        """
...
def mainMultiClasse(loss=CESM(), bruit=False, neuron = 100, classes=10, niter=100, gradient_step=0.01, display=False):
    """
    ...
    print(f"\nMulti-classe avec {type(loss).__name__}, activation TanH, bruit={bruit} :")
    print("Temps d'exécution: ", time.time() - time_start, "secondes")
    print("Score de bonne classification: ", np.mean( ypred == ytest ))
    if display:
        net = [linear_1, sigmoide, linear_2, sigmoide]
        
        # Création et test sur un réseau de neurones non linéaire
        time_start = time.time()
        
        nonlin2 = NonLin2()
        nonlin2.SGD(net, loss, xtrain, ytrain, batch_size=len(ytrain), niter=niter, gradient_step=gradient_step)
        
        # Test sur les données d'apprentissage
        ypred2 = nonlin2.predict(xtest)
        
        print(f"\nMulti-classe avec {type(loss).__name__}, activation Sigmoide, bruit={bruit} :")
        print("Temps d'exécution: ", time.time() - time_start, "secondes")
        print("Score de bonne classification: ", np.mean( ypred2 == ytest ))

        plt.figure()
        plt.plot(nonlin.list_loss, label="Activation TanH")
        plt.plot(nonlin2.list_loss, label="Activation Sigmoid")
        plt.legend()
        plt.title("Evolution du loss")
        plt.show()
...
def mainAutoEncodeur(loss=BCE(), bruit=False, neuron = 100, classes=10, niter=500, gradient_step=1e-4):
    ...
    plt.savefig(f"resultat/auto_encodeur_{type(loss).__name__}_{bruit}.pdf")
    plt.show()  
...
if (__name__ == "__main__"):
    ...
    # test du comparaison des différents fonction d'activation
    # mainMultiClasse(loss=MSELoss(), bruit=bruit, neuron = 100, classes=10, niter=n_iter, gradient_step=gradient_step, display=True)
